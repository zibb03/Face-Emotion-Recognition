...
def grabcut():
    ...
    img = cv2.imread('C:/Users/user/PycharmProjects/OpenCV/persontwo.jpg')
    img2 = img.copy()
    ...
    while True:
        ...
        if k == 27: '''esc누르면 꺼짐'''
            break
        if k == ord('0'):
            print('왼쪽 마우스로 제거할 부분을 표시한 수 n을 누르세요')
            value = DRAW_BG
        elif k == ord('1'):
            ...
            mask = np.zeros(img.shape[:2], dtype = np.uint8)
            output = np.zeros(img.shape, np.uint8)
            print('0:제거할 배경선택, 1:복월할 전경선택, n:적용하기, r:리셋')
        elif k == ord('n'):
            ...
            print('0:제거할 배경선택, 1:복원할 전경 선택, n:적용하기, r:리셋')
